...
public class DHT extends DHTBase implements IDHTResource, IDHTNode, IDHTBackground {
	...
	protected NodeInfo closestPrecedingFinger(NodeInfo info, int id) throws Failed {
		...
		if (localInfo.equals(info)) {
			...
			if (IRouting.USE_FINGER_TABLE) {
				...
				return client.predFinger(info, id);
			} else {
				/*
	...
	public void join(String uri) throws Failed, Invalid {
		 ...
		 * from us.
		 */
		URI ub = UriBuilder.fromPath(uri).build();
	...
	public void listenOn(String key, EventListener listener) throws DHTBase.Failed {
		/*
		 * Register a listener for new bindings under key, at the node where
		 * those bindings are stored. The event source should be registered in
		 * the state object, so the client can shut down the event stream at
		 * this point. The client will also need to contact the server to
		 * request that event generation be stopped at the server (for this
		 * client and key). The client should send its own node id to identify
		 * itself (for both the listen on and listen off requests).
		 */
	...
	public void listenOff(String key) throws DHTBase.Failed {
		/*
		 * Stop listening for new binding events for this key. Need to do a Web
		 * service call to the server node, to stop event generation, as well as
		 * close the event source here at the client.
		 */
		int id = DHTBase.NodeKey(key);
		client.listenOff(info, id, key);
	...
	public synchronized void clearListenersForNode(int predId) {

		Iterator<Entry<Integer, Map<String, EventOutput>>> iterator = state.getOutputs().entrySet().iterator();

		while (iterator.hasNext()) {
			Entry<Integer, Map<String, EventOutput>> entry = iterator.next();
			if ((entry.getValue()) != null || entry.getValue().isEmpty()) {
				try {
					clearListener(predId, entry.getValue(), entry.getKey());
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	public void clearListener(int predId, Map<String, EventOutput> entry, int removeKey) {

		Iterator<Map.Entry<String, EventOutput>> iterator = entry.entrySet().iterator();

		while (iterator.hasNext()) {
			Map.Entry<String, EventOutput> currentEntry = iterator.next();
			String key = currentEntry.getKey();
			OutboundEvent.Builder oeb = new OutboundEvent.Builder();
			OutboundEvent event = oeb.name("Clear Listener Event").id("-1").data(String.class, key).build();
