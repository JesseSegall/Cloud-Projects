...
public interface IDHTResource {
	...
	/*
	 * Exposing the finger table in the node API. Alternatively search for
	 * predecessor tail-recursively, but be sure to redirect HTTP to the next
	 * node.
	 */
	 ...
	 * key-value pairs is transferred from the successor (all bindings at the
	 * successor up to and including the key of the new predecessor). The
	 * predecessor supplies its own bindings, to be backed up at the successor.
	 */
	public TableRep notify(TableRep predDb);

	/*
	 * The operations for actually accessing the underlying hash table for a
	 * node.
	 */
	...
	public void addNet(String key, String val) throws Failed;

	public void deleteNet(String key, String val) throws Failed;
